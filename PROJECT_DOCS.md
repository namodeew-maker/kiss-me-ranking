...
ความหมาย: รายการบันทึกการหักจากบัญชีรางวัลของลูกค้าแต่ละครั้ง
...
เลือกสิทธิ์ของลูกค้าจากตาราง
  ↓
กรอกยอดที่จะหักจากบัญชีรางวัลของลูกค้าในครั้งนั้น + ใส่ note
  ↓
...
- ช่อง `amount` คือยอดเต็มที่ต้องการหักจากบัญชี Cashback ของลูกค้า
- ถ้าเลือก `reuse` ระบบจะหักยอดเต็มตามจำนวนที่กรอก
- ถ้าเลือก `withdraw` ระบบจะหักยอดเต็มตามจำนวนที่กรอกเช่นกัน แต่ยอดที่ลูกค้าได้รับเงินจริง = `amount * 0.90`
- ยอดถอนสุทธิที่หน้า Admin แสดงจะคำนวณจากยอด Cashback คงเหลือจริงในตอนนั้น ไม่ได้ล็อกตายจากวิธีใช้ครั้งก่อน
...
กรณีลูกค้าใช้ **Cashback 300 บาท** แบบใช้ซ้ำ:
...
กรณีลูกค้าถอน **Cashback 1,000 บาท** เป็นเงินสด:

```text
ยอดเดิม 5,000
ถอนครั้งที่ 1 = 1,000
ระบบหัก 10% ตอนจ่ายเงินจริง
ลูกค้าได้รับเงินจริง = 900
ยอดบัญชี Cashback คงเหลือ = 4,000
```
...
|------|------------|-----------|--------|
| `withdraw` (ถอนเงินสด) | หัก 10% ตอนจ่ายจริงทุกครั้ง | ถ้าหักจากบัญชี 1,000 ฿ ลูกค้าได้รับ 900 ฿ | ลูกค้าต้องการเงินสด |
| `reuse` (ใช้ซ้ำ) | ไม่หัก | 5,000 ฿ เต็มจำนวน | ลูกค้านำกลับมาใช้บริการ |

ระบบบันทึก `claim_mode` ทุกครั้งที่แอดมินตัดยอด Cashback ลง `lottery_reward_claims` โดย `amount` จะเป็นยอดที่ถูกหักจากบัญชี Cashback ต้นทางเสมอ
